			...
			if wc_server.enable_image_sync:
				...
				if item_dirty or fields_updated:
					...
					try:
						...
						error_text = cstr(err)
						maintain_stock_error = (
							"Maintain Stock" in error_text or "Lager verwalten" in error_text
						)
						if not maintain_stock_retried and (
							item.item.is_stock_item != maintain_stock_original or maintain_stock_error
						):
		...
		if item and woocommerce_product:
			...
			if wc_server.item_field_map:
				...
				for map in wc_server.item_field_map:
					...
					if woocommerce_field_name in ("$.name", "name"):
						frappe.logger("woocommerce_fusion").warning(
							(
								...
								f"{woocommerce_field_name} (Item {item.item.name})"
							)
						)
						continue
					...
					# We expect woocommerce_field_name to be valid JSONPath
					jsonpath_expr = parse(woocommerce_field_name)
